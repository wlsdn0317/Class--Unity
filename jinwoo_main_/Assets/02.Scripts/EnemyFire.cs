...
public class EnemyFire : MonoBehaviour
{
    ...
    void Update()
    {
        //공격 신호가 들어오면 실행
        //추가적인 조건 부여
        //재장전 중이 아니면서 공격신호가 들어오면 공격
        if (!isReload && isFire)
        {
            //Time.time은 겜 플레이 이후 실행된 시간
    ...
    IEnumerator ShowMuzzleFlash()
    {
        ...
        //기준벡터 (1,1,1) 인 벡터에 스케일을 곱해서 크기를 변경하는 것
        //Vector3(1,1,1)*2 = (2,2,2)가 됨
        muzzleFlash.transform.localScale = Vector3.one * Random.Range(1f, 2f);

        Vector2 offset = new Vector2(Random.Range(0, 2), Random.Range(0, 2)) * 0.5f;
        //세이더 내에서 사용되는 offset 값에
        //위에서 만든 offset값을 전달하기 위함
        //_MainTex 라는 명칭은 세이더 자체에서 만들어진 것으로
        //사용자가 변경 불가
        muzzleFlash.material.SetTextureOffset("_MainTex", offset);

        //머즐플레시 생성되는 시간을 0.05~0.2초 랜덤하게 설정
